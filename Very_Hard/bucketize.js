...
the phrase can't be bucketized, and the returned array has to be empty.
*/
...
function bucketize(str, n) {
    ...
    // Check if any word is too long - if so, return empty array immediately
    ...
    for (const word of words) {
        let currAdd = currStr === '' ? word : currStr + " " + word;
    ...
    // Add the last bucket if it has content
    if (currStr.length > 0) bucket.push(currStr);
